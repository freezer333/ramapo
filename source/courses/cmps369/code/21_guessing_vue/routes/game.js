...
router.post("/:gameId", function(req, res) {
    ...
    req.db.find({ _id: req.params.gameId }, function(err, docs) {
        if (err) {
            ...
            req.db.update({ _id: req.params.gameId }, { $set: { complete: true }, $push: { guesses: guess } });
